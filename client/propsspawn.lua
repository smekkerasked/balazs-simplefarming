...
function FarmProp(v)
    ...
    if tonumber(ESX.PlayerData.job.grade) >= v.rang and ESX.PlayerData.job.name == v.job or v.job == nil then
        IsPickingUp = true
        TriggerServerEvent("balazs-simplefarmingzone:server:prop:start", Source, v.requireditem, Activ.rewards.item,
...
RegisterNetEvent("balazs-simplefarmingzone:props:start")
AddEventHandler("balazs-simplefarmingzone:props:start", function()
    StartFarm()
end)

RegisterNetEvent("balazs-simplefarmingzone:props:cantfarm")
AddEventHandler("balazs-simplefarmingzone:props:cantfarm", function()
    Wait(1000)
    IsPickingUp = false
end)
